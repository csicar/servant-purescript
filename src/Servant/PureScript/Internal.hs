...
defaultSettings = Settings {
  ...
  , _standardImports = importsFromList
        ...
        , ImportLine "Control.Monad.Except" (Set.fromList ["except", "runExcept", "throwError", "withExcept"])
        , ImportLine "Effect.Aff.Class" (Set.fromList [ "class MonadAff", "liftAff" ])
        , ImportLine "Foreign.Generic" (Set.fromList ["Foreign", "decode", "encode"])
        , ImportLine "Foreign" (Set.fromList ["ForeignError(..)"])
        , ImportLine "Global" (Set.fromList ["encodeURIComponent"])
        , ImportLine "Foreign" (Set.fromList ["unsafeToForeign"])
        , ImportLine "Affjax.ResponseFormat" (Set.fromList ["json"])
        , ImportLine "Affjax" (Set.fromList [ "defaultRequest", "printError", "request", "Error" ])
        , ImportLine "Data.Nullable" (Set.fromList [ "toNullable" ])
        , ImportLine "Data.List.Types" (Set.fromList [ "NonEmptyList" ])
        , ImportLine "Prim" (Set.fromList [ "String" ]) -- For baseURL!
        , ImportLine "Data.Maybe" (Set.fromList [ "Maybe(..)", "fromMaybe"])
        , ImportLine "Data.Either" (Set.fromList [ "Either(..)", "either"])
